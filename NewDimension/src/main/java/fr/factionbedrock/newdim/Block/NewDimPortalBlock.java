...
public class NewDimPortalBlock extends Block
{
	...
	public boolean trySpawnPortal(IWorld worldIn, BlockPos pos) {
		NewDimPortalBlock.Size aetherPortalSize = this.isPortal(worldIn, pos);
	...
	public NewDimPortalBlock.Size isPortal(IWorld world, BlockPos pos) {
		NewDimPortalBlock.Size aetherPortalSizeX = new NewDimPortalBlock.Size(world, pos, Axis.X);
		if (aetherPortalSizeX.isValid() && aetherPortalSizeX.portalBlockCount == 0) {
			return aetherPortalSizeX;
		}
		else {
			NewDimPortalBlock.Size aetherPortalSizeZ = new NewDimPortalBlock.Size(world, pos, Axis.Z);
		...
		if(!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss()) {
			if(entity.func_242280_ah()) {
				...
				if(serverworld != null) {
					...
					RegistryKey<World> where2go = entity.world.getDimensionKey() == ModDimensions.NEW_DIMENSION ? World.OVERWORLD : ModDimensions.NEW_DIMENSION;
	...
	public static class Size {
		...
		protected boolean isEmptyBlock(BlockState pos) {
			...
			return pos.isAir() || block == Blocks.WATER || block == Registration.NEWDIM_PORTAL.get();
		}

...
public class NewDimGoldenAercloudFeature extends Feature<NoFeatureConfig>
{

    public NewDimGoldenAercloudFeature(Codec<NoFeatureConfig> codec)
    {
        super(codec);
    }
                    ...
                    for (int z = position.getZ(); z < position.getZ() + rand.nextInt(2) + 3; ++z) {
                        ...
                        if (reader.isAirBlock(newPosition)) {
                            if (Math.abs(x - position.getX()) + Math.abs(y - position.getY()) + Math.abs(z - position.getZ()) < 4 + rand.nextInt(2)) {
                                this.setBlockState(reader, newPosition, Registration.NEWDIM_GOLDEN_AERCLOUD.get().getDefaultState());
                            }
                        }
                    }
                }

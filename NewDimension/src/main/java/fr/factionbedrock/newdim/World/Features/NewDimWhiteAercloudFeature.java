...
public class NewDimWhiteAercloudFeature extends Feature<NoFeatureConfig>
{

    public NewDimWhiteAercloudFeature(Codec<NoFeatureConfig> codec)
    {
        super(codec);
    }
    ...
    public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, NoFeatureConfig config)
    {
        ...
        for (int amount = 0; amount < 16; ++amount) {
            ...
            position = position.add(xOffset, yOffset, zOffset); //offset
                    ...
                    for (int z = position.getZ(); z < position.getZ() + rand.nextInt(2) + 3; ++z) {
                        ...
                        if (reader.isAirBlock(newPosition)) {
                            if (Math.abs(x - position.getX()) + Math.abs(y - position.getY()) + Math.abs(z - position.getZ()) < 4 + rand.nextInt(2)) {
                                this.setBlockState(reader, newPosition, Registration.NEWDIM_WHITE_AERCLOUD.get().getDefaultState());
                            }
                        }
                    }
                }

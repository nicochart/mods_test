...
{
	public static Block PortalBlock = Blocks.GLOWSTONE; //Block to create the portal (if edited: edit it in Block.NewDimPortalBlock too)
	
    protected final ServerWorld world;

    public NewDimTeleporter(ServerWorld worldIn)
    {
        this.world = worldIn;
    }
    ...
    public Optional<TeleportationRepositioner.Result> getExistingPortal(BlockPos pos)
    {
        ...
        Optional<PointOfInterest> optional = poiManager.getInSquare((poiType) ->
                poiType == NewDimPOI.NEWDIM_PORTAL_POI.get(), pos, 128, PointOfInterestManager.Status.ANY).sorted(Comparator.<PointOfInterest>comparingDouble((poi) ->
    ...
    public Optional<TeleportationRepositioner.Result> makePortal(BlockPos pos, Direction.Axis axis)
    {
        ...
        if (d0 == -1.0D)
        {
            ...
            for(int l1 = -1; l1 < 2; ++l1)
            {
                for(int k2 = 0; k2 < 2; ++k2)
                {
                    ...
                    {
                        BlockState blockstate1 = i3 < 0 ? PortalBlock.getDefaultState() : Blocks.AIR.getDefaultState();
        ...
        for(int k1 = -1; k1 < 3; ++k1)
        {
            for(int i2 = -1; i2 < 4; ++i2)
            {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3)
                {
                    ...
                    this.world.setBlockState(mutablePos, PortalBlock.getDefaultState(), 3);
                }
            }
        }

        BlockState newdimPortal = Registration.NEWDIM_PORTAL.get().getDefaultState().with(NewDimPortalBlock.AXIS, axis);
        ...
        for(int j2 = 0; j2 < 2; ++j2)
        {
            for(int l2 = 0; l2 < 3; ++l2)
            {
                ...
                this.world.setBlockState(mutablePos, newdimPortal, 18);
            }
        }
    ...
    {
        boolean isNewDim = destWorld.getDimensionKey() == ModDimensions.NEW_DIMENSION;
        if (entity.world.getDimensionKey() != ModDimensions.NEW_DIMENSION && !isNewDim)
        {
            return null;
        }
        else
    ...
    protected Optional<TeleportationRepositioner.Result> getOrMakePortal(Entity entity, BlockPos pos)
    {
        ...
        {
            Direction.Axis portalAxis = this.world.getBlockState(entity.field_242271_ac).func_235903_d_(NewDimPortalBlock.AXIS).orElse(Direction.Axis.X);
